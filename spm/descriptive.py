...
class descriptive:
    ...
    def correlation(df):
        ...
        temp_data = df.groupby("id").agg({'estimated_str': 'sum', 'distance': 'mean'})
        print(temp_data.corr())

        print(pearsonr(temp_data["estimated_str"], temp_data["distance"]))
        print(spearmanr(temp_data["estimated_str"], temp_data["distance"]))